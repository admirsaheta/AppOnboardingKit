...
class AnimatedBarView: UIView {
    ...
    private lazy var  backgrounBarView: UIView = {
        let view = UIView()
    ...
    private func setupAnimator(){
        animator = UIViewPropertyAnimator(
            duration: 3.0, curve: .easeInOut, animations: {
    ...
    private func layout(){
        addSubview(backgrounBarView)
        backgrounBarView.addSubview(foregroundBarView)
        backgrounBarView.snp.makeConstraints { make in
            make.edges.equalTo(self)
            
        }
        foregroundBarView.snp.makeConstraints { make in
            make.edges.equalTo(backgrounBarView)
        }
    }
